...
md"""
## C03/w03: Identifying safe loans with decision trees
...
begin
  struct F_ST
    names::Vector{Symbol}
    st_map::Dict{Symbol, DataType}

    function F_ST(names::Vector{Symbol}, st_map::Dict{Symbol, DataType})
      @assert length(names) ≥ 1
      @assert length(st_map) ≥ 0
      new(names, st_map)
    end
  end

  ## subset of features whose scientific types need to be coerced
  ...
  features = F_ST(Features, Features_ST)

  const Target_ST = Dict{Symbol, DataType}(
    Target => Multiclass{2},
  )

  target = F_ST([Target], Target_ST)
end
...
begin
  safe_loans_raw = loans[loans[!, target.names[1]] .== 1, :]
  risky_loans_raw = loans[loans[!, target.names[1]] .== -1, :]
...
function dt_runner(df::DF;
    features=features, target=target,
    max_depth=-1) where {DF <: Union{DataFrame, SubDataFrame}}
  X, y = data_prep_Xy(df, features, target)
...
# function dt_runner_alt(df::DF;
#     features=features, target=traget
...
begin
  dt_model = dt_runner(train_data; max_depth=6)
...
begin
  dt_small_model = dt_runner(train_data; max_depth=2)
...
begin
  dt_big_model = dt_runner(train_data; max_depth=10)
...
begin
  n_validation_data = data_prep(validation_data, features)

  valid_safe_loans = filter(target.names[1] => ==(1), n_validation_data)
  valid_risky_loans = filter(target.names[1] => ==(-1), n_validation_data)
...
begin
  ŷ_mode = predict_mode(dt_model,
    select(sample_valid_data, Features))
  println(ŷ_mode)

  perc_correct_preds = sum(ŷ_mode .== sample_valid_data.safe_loans) / size(sample_valid_data)[1]
...
#   ixes_ = filter(((_, v)=t) -> v,
#       collect(enumerate(sample_valid_data.safe_loans .== 1))) |>
#     a_ -> map(t -> t[1], a_)
